...
from config import BSV_DUST_LIMIT, SATOSHIS_PER_BSV, SATOSHI_PRECISION
...
def _validate_and_adjust_bounds(total_amount, address_count, min_amount, max_amount, avg_amount):
    """
    ...
    if min_total_required > total_amount:
        ...
        min_amount = total_amount * Decimal('0.8') / address_count  # Use 80% to leave buffer
    ...
    if worst_case_remainder > acceptable_max_remainder:
        ...
        print(f"ℹ️  Adjusted maximum amount to {max_amount} BSV to prevent excessive last address")
    ...
    if adjusted_min_total >= total_amount:
        ...
        min_amount = total_amount * Decimal('0.6') / address_count  # Use 60% for conservative approach
    ...
    if max_amount <= min_amount * Decimal('1.1'):  # At least 10% difference
        ...
        print(f"ℹ️  Created balanced range around average: {min_amount} - {max_amount} BSV")
...
def distribute_amounts_random_smart(total_amount, address_count, min_amount, max_amount):
    """
    ...
    for i in range(address_count):
        ...
        if remaining_addresses == 2:  # Next to last address
            max_to_leave_reasonable = min(target_last_amount, max_amount * Decimal('1.5'))
...
def validate_distribution_params(total_amount, address_count, min_amount, max_amount):
    ...
    if max_amount >= total_amount:
        return False, f"Maximum amount must be less than total amount ({total_amount} BSV)."
    
    # Check if minimum is greater than maximum
    if max_amount <= min_amount:
        return False, f"Maximum amount must be greater than minimum amount ({min_amount} BSV)."
    
    # Check if minimum total exceeds available amount
    min_total = min_amount * address_count
    if min_total > total_amount:
        return False, f"Minimum total ({min_total} BSV) exceeds available amount ({total_amount} BSV)."
    
    # Check dust limit
    min_sats = int(min_amount * SATOSHIS_PER_BSV)
    if min_sats <= BSV_DUST_LIMIT:
        min_dust_bsv = Decimal(BSV_DUST_LIMIT) / SATOSHIS_PER_BSV
        return False, f"Minimum amount must be greater than {min_dust_bsv} BSV ({BSV_DUST_LIMIT} satoshis)."
...
def analyze_distribution_quality(amounts, min_bound, max_bound, total_amount):
    ...
    within_bounds = sum(1 for amount in amounts[:-1] if min_bound <= amount <= max_bound)
    ...
    return {
        ...
        'total_distributed': sum(amounts),
        'distribution_accuracy': abs(sum(amounts) - total_amount) < Decimal('0.00000001')
    } 